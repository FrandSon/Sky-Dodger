function createAirplaneMesh() {
  ...
  // Add keyboard state tracking (put this in your initialization)
  // const keyboardState = {
  //   KeyW: false,
  //   KeyA: false,
  //   KeyS: false,
...
class Airplane {
  ...
  tick(deltaTime) {
    ...
    if (game.status === 'playing') {
      // Get keyboard input from UI
      const input = ui.getKeyboardInput();

      // Update plane speed based on horizontal input
      ...
      // Update plane position with smooth interpolation
      ...
      // Update plane rotation (banking effect)
      ...
      if (input.shoot) {
        airplane.shoot();
        ui.resetShootState();  // Prevent continuous firing
      }
      ...
      if (game.fpv) {
        ...
        // Third-person view
        camera.fov = 60;  // Fixed FOV
        camera.updateProjectionMatrix();

        // Smooth camera follow on Y-axis
        camera.position.y += (this.mesh.position.y - camera.position.y) * deltaTime * world.cameraSensivity;

        // Add horizontal follow if needed:
        // camera.position.x += (this.mesh.position.x - camera.position.x) * deltaTime * world.cameraSensivity;
      }
    }
...
class UI {
  ...
  handleKeyDown(event) {
    ...
    // Enter key now handled through keyboardState
    if (event.code === 'Enter') {
      if (game.fpv) {
        setSideView();
      } else {
    ...
    // Add replay functionality to R key
    if (event.code === 'KeyR' && game.status === 'waitingReplay') {
      this.handleReplay();
    }
  }
  ...
  // New method to handle replay functionality
  handleReplay() {
    resetMap();
  ...
  getKeyboardInput() {
    return {
      ...
      vertical: (this.keyboardState.KeyW || this.keyboardState.ArrowUp) ? 1 :
        (this.keyboardState.KeyS || this.keyboardState.ArrowDown) ? -1 : 0,

      // Horizontal: A/D or Left/Right arrows
      horizontal: (this.keyboardState.KeyD || this.keyboardState.ArrowRight) ? 1 :
        (this.keyboardState.KeyA || this.keyboardState.ArrowLeft) ? -1 : 0,

      // Shooting: Spacebar
      shoot: this.keyboardState.Space
    };
  }
